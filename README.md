...
Apesar do programa utilizar somente 1 busca, está implementada outra busca para utilização caso o desenvolvedor queira alterar.  
O código escrito na linguagem __Java__ que utiliza a biblioteca `AIMA` para a implementação da Inteligência Artificial.
...
> Obs: O arquivo `.jar` da biblioteca `AIMA` deve estar na pasta lib/ 
...
Existe um *release* contendo o `.jar` que executa o programa sem necessitar do código fonte.
